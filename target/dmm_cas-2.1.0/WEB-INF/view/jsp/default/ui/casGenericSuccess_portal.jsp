...
<nav class="navbar navbar-default ">
  <div class="container">
    <header class="login-header">
      ...
      <a class="login-header-infor" href="#">学业预警帮扶系统</a>
      <nav>
      <a >你好，${name }</a> 
...
<div class="mu-list">
  ...
  <c:forEach var="sy" items="${sys}" varStatus="status" begin="${index.index*4}" end="${index.index*4+3}">
    ...
    <c:if test="${sy.istrue eq '1'}"> 
     <a href="${ fn:split(sy.url_,';')[0]}" target="_blank"><div class="mu-img-bg mu-img-default mu-img-${fn:replace(sy.shiroTag,':', '-')} " >${sy.name}</div></a>
	</c:if> 
        <c:if test="${sy.istrue eq '0'}"> 
    <div class="mu-img-bg mu-img-default mu-img-${fn:replace(sy.shiroTag,':', '-')}">${sy.name}<span style="color:red;font-size: smaller;">(未启用)</span></div>
	</c:if>
      </div>
  </c:forEach>
   </div>
